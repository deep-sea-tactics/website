<script>
	...
	let upgradeClickMania = () => {
		if (clicks >= clickManiaCost) {
			...
			clicksPerClick++;
			clickManiaCost = Math.round(
				ogClickManiaCost * Math.pow(globalUpgradeMulti, clickManaiaLevel)
			);
		}
	};
	...
	let upgradeClickParadise = () => {
		if (clicks > clickParadiseCost) {
			...
			clicksPerClick += 10;
			clickParadiseCost = Math.round(
				ogClickParadiseCost * Math.pow(globalUpgradeMulti, clickParadiseLevel)
			);
		}
	};
...
<button on:click={upgradeClickParadise}
	>Click Paradise | +10 Clicks Per Click | ${clickParadiseCost}</button
>
<button on:click={upgradeClickCrazy}
	>Click Crazy | +10 Clicks Per Click Per Click | ${clickCrazyCost}</button
>
